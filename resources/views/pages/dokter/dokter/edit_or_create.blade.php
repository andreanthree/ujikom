...
@section('breadcrumbs')
  <div class="breadcrumb-item"><a href="{{ route('kategori_dokter.index') }}">Data {{$dataPage['title']}}</a></div>
...
@section('content')
  ...
  @component('components.form')
    ...
    @if(isset($item))
      ...
      @slot('method', 'PUT')
      @slot('form_action', 'dokter.update')
      @slot('update_id', $item->kode_dokter)
    @else 
      @slot('form_method', 'POST')
      @slot('form_action', 'dokter.store')
    @endif
    ...
    @foreach ($dataPage['formData'] as $val)
      ...
      @endcomponent
    @endforeach
    @foreach ($dataPage['formUser'] as $val)
      @component('components.input-field')
        @slot('input_label', $val['label'])
        @slot('input_type', $val['type'])
        @slot('input_name', $val['name'])
